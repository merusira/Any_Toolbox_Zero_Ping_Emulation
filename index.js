...
class RivalEmulation {
    constructor(mod) {
        /* 
         * Check for required library, hsdn library this mod may use has object autism4.
         * If that library is not found, even though you have hsdn library, they may have
         * updated it in a way that this mod would not detect in the original way,
         * thus we must revert to another library backup stored on this mods server
         */
         if (!mod.require.library.autism4) {
            console.error('Rival will not load due to the wrong library mod being installed.');
            //console.log('Available library properties:', Object.keys(mod.require.library || {}));
            console.log('If it has auto downloaded the one from HSDN and it still does not work,');
			console.log('Then you need to download my backup manually from here:');
			console.log('https://github.com/merusira/library');
			console.log('Just put it in your mods folder as you would any other mod.');
			console.log('You must DISABLE the other library (if you have it) while you use Rival.');
			console.log('glhf')
            return;
        }
